...
def _audio_decode_errors_as_invalid() -> Iterator[None]:
    """Context manager mapping audio decoding failures to InvalidArgument (HTTP 400)."""
    try:
        yield
...
def _strip_words(segments: Iterable[Segment]) -> Iterable[Segment]:
    """Drop per-word timestamps lazily after speaker merging is complete."""
    for seg in segments:
        seg.words = None
        yield seg
...
def _language_mass(runs: list[tuple[str, list[tuple[float, float]]]]) -> dict[str, float]:
    """Calculate total speech duration for each language across all decode runs."""
    mass: dict[str, float] = {}
...
def _majority_run_language(runs: list[tuple[str, list[tuple[float, float]]]]) -> str:
    """Return the language covering the most total speech duration."""
...
def _synthesize_multilang_info(
    ...
    """Build summary TranscriptionInfo object covering all language runs."""
    mass = _language_mass(runs)
...
class FasterWhisperHandler:
    """Handler managing Whisper model execution, diarization pipeline integration, and multilang runs."""
    ...
    def _warm_decode(whisper: WhisperModel) -> None:
        """Run throwaway decode on silence to trigger CUDA kernel compilation."""
    ...
    def transcribe_audio(
        ...
        """Transcribe audio request and format response."""
    ...
    def translate_audio(self, request: TranslationRequest) -> WhisperResponse:
        """Translate audio file to English and format response."""
    ...
    def _decode_options(request: DecodeParams, word_timestamps: bool) -> dict:
        """Extract transcribe() parameters from request model."""
        return dict(
