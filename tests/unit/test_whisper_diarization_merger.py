...
def test_diarization_segments_consumed_incrementally():
    ...
    list(merge_whisper_diarization([whisper_segments[0]], segment_generator()))
    assert consumed_count == 3

    consumed_count = 0

    list(merge_whisper_diarization(whisper_segments[:2], segment_generator()))
    ...
    assert result[1].speaker in ("A", "B")
...
def test_segment_spanning_speaker_change_is_split():
    ...
    result = list(merge_whisper_diarization(whisper_segments, diarization_segments))
...
def test_border_word_straddling_turn_boundary_does_not_split():
    ...
    whisper_segments = [
        DummyWhisperSegment(
            ...
            words=[
                DummyWord(start=110.33, end=110.77, word=" Hallo"),
                DummyWord(start=110.77, end=111.19, word=" zusammen"),
            ],
        )
    ]
    diarization_segments = [
    ...
    whisper_segments = [
        DummyWhisperSegment(
            ...
            words=[
                ...
                DummyWord(start=3.0, end=3.1, word=" mid"),
...
def test_words_in_pad_regions_snap_to_turn():
    ...
    whisper_segments = [
        DummyWhisperSegment(
            ...
            words=[
                DummyWord(start=0.7, end=0.95),
                DummyWord(start=1.5, end=2.0),
                DummyWord(start=5.05, end=5.25),
            ],
        )
    ]
    diarization_segments = [DiarizationSegment(segment=Segment(1, 5), speaker="A")]
...
def test_word_between_two_turns_snaps_to_closer():
    ...
    whisper_segments = [
        DummyWhisperSegment(
            ...
            words=[
                DummyWord(start=0.2, end=0.8),
                DummyWord(start=1.1, end=1.15),
                DummyWord(start=2.0, end=4.0),
            ],
        )
    ]
    diarization_segments = [
    ...
    assert [word.speaker for word in words] == ["A", "A", "B"]
...
def test_word_far_from_any_turn_stays_none():
    ...
    whisper_segments = [
        DummyWhisperSegment(
            ...
            words=[
                DummyWord(start=0.2, end=0.8),
                DummyWord(start=2.0, end=2.1),
                DummyWord(start=4.2, end=4.8),
            ],
        )
    ]
    diarization_segments = [
    ...
    words = [word for seg in result for word in (seg.words or [])]
    assert [word.speaker for word in words] == ["A", None, "B"]
    assert [seg.speaker for seg in result] == ["A", "B"]
